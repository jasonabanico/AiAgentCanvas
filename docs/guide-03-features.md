...
| **Event triggers** | Proactive agent engagement through scheduled (cron), file-watch, and webhook triggers. Triggers fire agent messages without user interaction, enabling monitoring, alerting, and event-driven automation. |
...
| **Evaluation** | Eval cases (a test input plus grading criteria) are stored in SQLite. Running a case sends the input to the raw chat client, independent of the tool pipeline, and grades the response with an LLM-as-judge pass, producing a score, pass/fail, and rationale. Agents can add cases, run a single case or a whole category as a regression suite, and pull pass-rate statistics over time. |
